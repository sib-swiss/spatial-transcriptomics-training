...
SpatialPlot(seu, cells.highlight = high_mt_spots) + NoLegend()
...
as.matrix(raw_counts[most_expressed, ]) |>
  t() |>
  as.matrix() |>
  boxplot(
    cex.axis = 0.5,
...
```{r}
saveRDS(seu,
        paste0("output/normalized_Posterior.rds"))
```

```{r}
seu2 <- Load10X_Spatial(data.dir = "raw_data/Anterior/",
                       slice = "Anterior")

seu2 <- seu2[, seu2$nFeature_Spatial > 500 & seu2$percent_mt < 32]

remove_feature <- rownames(seu2) %in% remove_genes
seu2 <- seu2[!remove_feature, ]
...
saveRDS(seu,
        paste0("output/normalized_Anterior.rds"))
